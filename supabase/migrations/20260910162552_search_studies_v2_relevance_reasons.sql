create or replace function public.search_studies_v2(
  ...
  scored as (
    select
      ...
      (
        ...
        + case when prepared.text_query is null then 0 else
            ts_rank_cd(prepared.busca_texto, prepared.text_query) * 100
    ...
    left join lateral (
      ...
      join public.passages pg on pg.id = sp.passage_id
      join public.books bk on bk.id = pg.book_id
      where sp.study_id = prepared.id
    ) ref on true
    left join lateral (
      select
        prepared.text_query is not null
          and to_tsvector('portuguese', coalesce(prepared.titulo, '')) @@ prepared.text_query
          as title_match,
        prepared.text_query is not null
          and to_tsvector('portuguese', array_to_string(coalesce(prepared.palavras_chave, '{}'::text[]), ' ')) @@ prepared.text_query
          as keyword_match,
        prepared.text_query is not null
          and to_tsvector('portuguese', coalesce(prepared.resumo, '')) @@ prepared.text_query
          as summary_match,
        prepared.text_query is not null
          and to_tsvector('portuguese', coalesce(prepared.conteudo, '')) @@ prepared.text_query
          as content_match,
        prepared.text_query is not null
          and exists (
            select 1
            from public.study_topics st
            join public.topics t on t.id = st.topic_id
            where st.study_id = prepared.id
              and to_tsvector('portuguese', coalesce(t.nome, '')) @@ prepared.text_query
          ) as topic_match,
        prepared.text_query is not null
          and exists (
            select 1
            from public.study_characters sc
            join public.characters c on c.id = sc.character_id
            where sc.study_id = prepared.id
              and to_tsvector('portuguese', coalesce(c.nome, '')) @@ prepared.text_query
          ) as character_match,
        prepared.text_query is not null
          and exists (
            select 1
            from public.study_series ss
            join public.series se on se.id = ss.series_id
            where ss.study_id = prepared.id
              and to_tsvector('portuguese', coalesce(se.nome, '')) @@ prepared.text_query
          ) as series_match
    ) flags on true
  )
  select
...
revoke all on function public.search_studies_v2(text,text,integer,integer,integer,text,text,text,text,text,boolean,integer,integer) from public;
grant execute on function public.search_studies_v2(text,text,integer,integer,integer,text,text,text,text,text,boolean,integer,integer) to anon, authenticated, service_role;
